...
(function run() {
  const base = {
    ...
    nowTs: 1_799_999_999,
    vaultAmount: 0,
    marketState: { status: 'Seeding', totalPool: 0, lockTimestamp: 1_800_000_000 },
  };
  ...
  assert.equal(
    validateCancelMarketInput({ ...base, marketState: { ...base.marketState, status: 'Open' } }),
    'MarketNotSeeding'
  );
  assert.equal(validateCancelMarketInput({ ...base, nowTs: base.marketState.lockTimestamp }), 'TooLateToCancel');
